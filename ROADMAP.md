...
| — | Scoring Breakdown Badges | **completed** | Color-coded 2-letter badges (ES/GD/OC/OT/TG/RG) on Results tab showing which scoring factors contributed to each prediction's points |
| 13 | Push Notifications | **cancelled** | Not planned |
| 13 | Stats & Analytics | **cancelled** | Not planned |
| 14 | i18n & Accessibility | **done** | Cookie-based locale, en/es dictionaries, all UI strings translated, LanguageSwitcher, skip-to-content, ARIA labels |
...
## Enhancement — Scoring Breakdown Badges

**Goal:** Give users visibility into how their points were calculated — which scoring factors contributed to each prediction's score.

**Deliverables:**
- [x] Results API (`GET /api/groups/:id/results`) enhanced to compute `calculateScore()` breakdown per prediction, loading the group's `scoringRules` for accurate factor evaluation
- [x] API response includes `breakdown` object with all 6 factor scores (`exactScore`, `goalDifference`, `outcome`, `oneTeamGoals`, `totalGoals`, `reverseGoalDifference`)
- [x] `BreakdownBadges` component — color-coded 2-letter badges displayed inline with each prediction's points:
  - **ES** (green) — Exact Score
  - **GD** (blue) — Goal Difference
  - **OC** (violet) — Outcome
  - **OT** (amber) — One Team's Goals
  - **TG** (cyan) — Total Goals
  - **RG** (rose) — Reverse Goal Difference
- [x] Each badge has a tooltip showing the full factor name and points (e.g., "One Team's Goals: +3")
- [x] Only badges with non-zero points are shown
- [x] All 182 tests passing, clean build

**Testable outcome:**
Open Results tab → expand a match → see colored badges next to each prediction showing exactly which scoring factors contributed (e.g., ES + GD + OC + OT + TG = 25 pts for an exact score hit).

---

## ~~Cycle 13 — Push Notifications~~ (Cancelled)

> Not planned.

---

## ~~Cycle 13 — Stats & Analytics~~ (Cancelled)
...
**Deliverables:**
- [x] Extract all user-facing strings into i18n resource files (`web/src/i18n/en.json`, `web/src/i18n/es.json`)
- [x] Language switcher in the UI (`LanguageSwitcher` component on Profile page)
- [x] Add at least one additional language — Spanish (`es`)
- [x] Accessibility improvements: skip-to-content link, `<html lang>` dynamic attribute, `role="status"` + `aria-live` on LiveBadge, `aria-label` on navigation, `<main id="main-content">` landmark
- [ ] Full WCAG 2.1 AA audit (partial — key landmarks and labels in place)

**Implementation details:**
- Custom lightweight i18n — no external dependency
- Cookie-based locale persistence (`locale` cookie, `max-age=1y`)
- Server components: `getLocale()` + `getT(locale)` pattern
- Client components: `useTranslation()` hook via React context
- Translation format: nested JSON, dot-path keys, `{param}` interpolation, pipe-separated plural forms
- ~25 UI files updated (all pages + all client components)
- Global vitest mock in `vitest.setup.ts` provides English translations to tests
