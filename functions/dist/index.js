...
async function syncLiveTimer(_timer, context) {
        ...
        if (data.synced?.length > 0) {
            for (const s of data.synced) {
                if (s.error) {
                    ...
                    context.log(`  ${s.contestName}: ${s.matchesUpserted} matches, ${s.predictionsScored} scored`);
                }
            }
        }
    }
    catch (error) {
        context.error(`Failed to call sync-live: ${error instanceof Error ? error.message : error}`);
    }
}
functions_1.app.timer("syncLiveTimer", {
    // Every 2 minutes
    schedule: "0 */2 * * * *",
